<div class="list-group-item d-flex justify-content-between align-items-start pt-3">
    ...
    <div class="w-100 ms-3">
        ...
        <div class="d-flex w-100 justify-content-between align-items-center">
            ...
            @endif
            @if (request()->route()->named('user.dislike.*'))
                <small class="text-body-secondary">
        ...
        <div class="d-flex w-100 justify-content-between align-items-center">
            @auth
                @if (request()->route()->named('admin.*'))
                    ...
                    @endcan
                @else
                    <span class="lead d-flex align-items-center">
                        @if($like = $record->likes()->forUser(Auth::user())->first())
                            <a href="{{ route('user.like.delete', $like) }}" class="text-decoration-none text-primary mx-1">
                        ...
                        @else
                            <a href="{{ route('user.like.create', $record) }}" class="text-decoration-none text-secondary mx-1">
                        ...
                        @endif
                        @if($dislike = $record->dislikes()->forUser(Auth::user())->first())
                            <a href="{{ route('user.dislike.delete', $dislike) }}" class="text-decoration-none text-primary mx-1">
                        ...
                        @else
                            <a href="{{ route('user.dislike.create', $record) }}" class="text-decoration-none text-secondary mx-1">
                                <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor" class="bi bi-hand-thumbs-down" viewBox="0 0 16 16">
                    ...
                    @can('update', $record)
                        <a href="{{ route('user.record.edit', $record) }}" class="btn btn-sm btn-warning mx-1">{{ __('button.edit') }}</a>
                    @endcan
                    <div class="lead d-flex align-items-center">
                            ...
                            <div class="dropdown my-1">
                                ...
                                <ul class="dropdown-menu">
                                    @forelse($playlists as $playlist)
                                        <li><a class="dropdown-item text-small" href="{{ route('playlist.record.update', [$playlist, $record]) }}"><small>{{ $playlist->name }}</small></a></li>
                                    @empty
                                        <li>Нет</li>
                                    @endforelse
                                </ul>
                        ...
                        @endisset
                        @if (request()->route()->named('user.playlist.show'))
